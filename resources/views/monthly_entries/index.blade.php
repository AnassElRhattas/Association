                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full table-auto">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Payer') }}</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($entries as $entry)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">{{ \Carbon\Carbon::createFromFormat('Y-m', $entry->month)->format('F Y') }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                <a href="{{ route('monthly_entries.edit', $entry) }}" class="text-indigo-600 hover:text-indigo-900">{{ __('Edit') }}</a>
                                                @if($entry->status != 'paid')
                                                <form method="POST" action="{{ route('monthly_entries.mark-paid', $entry) }}" class="inline">
                                                    @csrf
                                                ...
                                                @endif -->
                                                <form method="POST" action="{{ route('monthly_entries.destroy', $entry) }}" class="inline" onsubmit="return confirm('{{ __('Are you sure?') }}')">
                                                    @csrf
                                                    @method('DELETE')
