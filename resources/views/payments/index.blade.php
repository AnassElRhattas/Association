                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                        <div class="bg-blue-50 p-4 rounded-lg">
                            <div class="text-blue-600 text-sm font-medium">{{ __('Total des paiements') }}</div>
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                        <div class="flex justify-center space-x-3">
                                            <a href="{{ route('students.payments.edit', [$student, $payment]) }}" class="text-indigo-600 hover:text-indigo-900" title="{{ __('Edit') }}" aria-label="{{ __('Edit') }}">
                                            ...
                                            @if($payment->status != 'paid')
                                            <form action="{{ route('students.payments.mark-paid', [$student, $payment]) }}" method="POST" class="inline" title="{{ __('Mark Paid') }}" aria-label="{{ __('Mark Paid') }}">
                                                @csrf
                                            ...
                                            <form action="{{ route('students.payments.mark-overdue', [$student, $payment]) }}" method="POST" class="inline" title="{{ __('Mark Overdue') }}" aria-label="{{ __('Mark Overdue') }}">
                                                @csrf
                                                <button type="submit" class="text-yellow-600 hover:text-yellow-800">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                                    </svg>
                                                    <span class="sr-only">{{ __('Mark Overdue') }}</span>
                                                </button>
                                            </form>
                                            @endif

                                            <form action="{{ route('students.payments.destroy', [$student, $payment]) }}" method="POST" class="inline" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce paiement ?')" title="{{ __('Delete') }}" aria-label="{{ __('Delete') }}">
                                                @csrf
                                                @method('DELETE')
